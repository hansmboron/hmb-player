...
class MyDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final UserModel user = UserModel.fromMap(GetStorage().read('user'));
          ...
          children: [
            UserAccountsDrawerHeader(
              currentAccountPicture: CircleAvatar(
                backgroundImage: CachedNetworkImageProvider(user.photoUrl ?? 'https://placekitten.com/150/150'),
              ),
              accountName: Text(user.name ?? 'Nome'),
              accountEmail: Text(user.email ?? 'email@email.com'),
            ),
            ListTile(
              selected: true,
