...
import 'widgets/box_player_widget.dart';
...
class PlaylistPage extends GetView<PlaylistController> {
  ...
  PlaylistPage({super.key, this.isLocal = false, this.isUserPlay = false});

  @override
  Widget build(BuildContext context) {
    final Size size = MediaQuery.of(context).size;
    final int orientation = MediaQuery.of(context).orientation.index;
    final String playlistTitle = snapshot?.get('title') ?? '';
    final String playlistId = snapshot?.id ?? '';

    return WillPopScope(
      onWillPop: () async {
        // delete local files from cache when close and stop player
        final String appDir = (await getTemporaryDirectory()).path;
        Directory(appDir).delete(recursive: true);
        return controller.audioPlayer.stop().then((value) => true);
      },
      child: Scaffold(
                ...
                children: [
                  Hero(
                    ...
                    child: SizedBox(
                      ...
                      height: size.height * (orientation == 0 ? 0.4 : 0.6),
                      child: isLocal || isUserPlay
                          ? Image.asset(
                              isUserPlay ? 'assets/images/my_playlist.jpg' : 'assets/images/bg.jpg',
                              fit: BoxFit.cover,
                            )
                          : CachedNetworkImage(
                  ...
                  Positioned(
                    ...
                    child: IconButton(
                        onPressed: () async {
                          ...
                          final String appDir = (await getTemporaryDirectory()).path;
                          Directory(appDir).delete(recursive: true);
                          await controller.audioPlayer.stop().then((value) => Get.back());
                        },
                  ...
                  builder: ((controller) {
                    return isLocal
                        ...
                        : FutureBuilder<QuerySnapshot>(
                            future: isUserPlay ? controller.getUserPlaylist() : controller.getRemoteAudios(snapshot?.id ?? ''),
                              ...
                              if (!snapshot.hasData) {
                                ...
                                return ListView(
                                  ...
                                  children: snapshot.data!.docs.map((d) {
                                    final AudioModel audio = AudioModel.fromDocument(d);
                                    return PlaylistTile(
                                      audio: audio,
                                      playlistName: playlistId,
                                      isLocal: isLocal,
